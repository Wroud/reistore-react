...
class Connector extends React.Component {
    constructor(props) {
        ...
        this.update = () => {
            ...
            const mapped = reistore_1.isScope((schema || store.schema))
    ...
    render() {
        ...
        if (store) {
            this.lastData = reistore_1.isScope(schema || store.schema)
                ? map((schema || store.schema).getState(store), props, store.state)
                : map((schema || store.schema).getState(store), props, store.state);
        }
...
function connect(map = (f => f), schema) {
    return (innerComponent) => (props) => (React.createElement(StoreProvider_1.StoreConsumer, null, store => (React.createElement(Consumer, null, context => (React.createElement(Connector, { schema: schema, map: map, store: store, parentContext: context, innerComponent: innerComponent, props: props }))))));
}
